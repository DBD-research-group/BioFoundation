...
def param_groups_lrd(
    ...
    layer_decay=0.75,
    decay_type: Literal["right", "inverse_normal", "normal"] = "right",
):
    """
    ...
    if decay_type == "right":
        ...
        x = np.linspace(
            -3, 3, num_layers + 1
        )  # Adjust spread to match the required range
        ...
        scaling_factors = (
            layer_decay ** position_counts[::-1]
        )  # Reverse to start from 0.75^13

        scaled_left = scaling_factors[
            :midpoint
        ]  # Use appropriate portion of scaling factors
