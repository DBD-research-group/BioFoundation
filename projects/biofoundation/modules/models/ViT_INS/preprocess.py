...
class AudioToImageConverter(object):
    ...
    def __init__(
        ...
        if self.freq_scale == "mel":
            self.mel_frequencies = mel_frequencies(
                n_mels=self.mel_bands + 1, fmin=self.mel_min_hz, fmax=self.mel_max_hz
            )

        self.global_min = None
        self.global_max = None
        self.bins = list(range(-200, 100))
        self.db_dist = {bin: 0 for bin in self.bins}
    ...
    def _samples_to_magnitude_spectrogram(self, samples):
        ...
        magnitudes = np.abs(spectrum) * 2.0 / window.sum()
    ...
    def _mel_scale(self, spectrogram):
        ...
        if spectrogram.ndim == 3 and spectrogram.shape[-1] == 1:
            spectrogram = spectrogram.squeeze(-1)  # Remove last dimension if it's 1

        # Ensure we have the right shape: [Time, Frequency]
    ...
    def _magnitude_to_db(self, spectrogram):
        ...
        db_spectrogram = log_spectrogram - 20.0 * np.log10(
            np.maximum(self.amin, self.ref_power_value)
        )
        ...
        if self.save_distribution is not None:
            for k in range(len(self.bins) - 1):
                lower = self.bins[k]
                upper = self.bins[k + 1]
                self.db_dist[lower] += int(
                    np.sum((db_spectrogram >= lower) * (db_spectrogram < upper))
                )
    ...
    def _db_to_uint8(self, spectrogram):
        ...
        spectrogram = (
            (spectrogram - self.min_db_value) / (self.max_db_value - self.min_db_value)
        ) * 255.0
    ...
    def _resize(self, spectrogram):
        ...
        if self.target_height is not None and self.target_width is not None:
            ...
            spectrogram = np.array(
                PIL.Image.fromarray(spectrogram).resize(
                    [self.target_width, self.target_height], resample=PIL.Image.BILINEAR
                )
            )
        ...
        # Did we just specify a height?
            ...
            spectrogram = np.array(
                PIL.Image.fromarray(spectrogram).resize(
                    [width_target, self.target_height], resample=PIL.Image.BILINEAR
                )
            )
        ...
        # Did we just specify a width?
            ...
            spectrogram = np.array(
                PIL.Image.fromarray(spectrogram).resize(
                    [self.target_width, height_target], resample=PIL.Image.BILINEAR
                )
            )
    ...
    def _transform_boxes(self, spectrogram, boxes, freq_scale=1.0, time_scale=1.0):
        ...
        if self.freq_scale == "mel":
            spec_high_freq = spectrogram_height_float - (
                (np.digitize(boxes[:, 0], bins=self.mel_frequencies, right=False) - 1)
                * freq_scale
            )
            spec_low_freq = spectrogram_height_float - (
                (np.digitize(boxes[:, 2], bins=self.mel_frequencies, right=False) - 1)
                * freq_scale
            )

        else:
            x_ref = np.linspace(
                start=0.0, stop=self.samplerate // 2, num=self.fft_length // 2
            )
            y_ref = np.linspace(
                start=0.0, stop=self.fft_length // 2, num=self.fft_length // 2
            )
            spec_high_freq = spectrogram_height_float - (
                np.interp(boxes[:, 0], x_ref, y_ref) * freq_scale
            )
            spec_low_freq = spectrogram_height_float - (
                np.interp(boxes[:, 2], x_ref, y_ref) * freq_scale
            )

        # Clip the values to ensure they are in range
        spec_start_sec = np.clip(spec_start_sec, 0.0, spectrogram_width_float - 1)
        spec_end_sec = np.clip(spec_end_sec, 0.0, spectrogram_width_float - 1)
        spec_high_freq = np.clip(spec_high_freq, 0.0, spectrogram_height_float - 1)
        spec_low_freq = np.clip(spec_low_freq, 0.0, spectrogram_height_float - 1)
    ...
    def __call__(self, samples, boxes=None):
        ...
        if boxes is not None and boxes.shape[0]:
            spectrogram_boxes = self._transform_boxes(
                spectrogram, boxes, freq_scale, time_scale
            )
...
def convert_file_list(args, file_list, wav2spec):
    for dirname, audio_name in tqdm.tqdm(file_list, leave=False):
        ...
        wav_path = os.path.join(args.data_root, dirname, audio_name)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--np_dir", type=str, default="")
    ...
    dirlist = [(os.path.join(args.data_root, d), d) for d in os.listdir(args.data_root)]
    dirlist = [(d_p, d) for (d_p, d) in dirlist if os.path.isdir(d_p)]
