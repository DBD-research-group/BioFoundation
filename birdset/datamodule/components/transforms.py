...
class BaseTransforms:
    ...
    def __init__(
        ...
        ), "Feature Extractor must be of type DefaultFeatureExtractor"
...
class BirdSetTransformsWrapper(BaseTransforms):
    ...
    def __init__(
        ...
        self.wave_aug = torch_audiomentations.Compose(
            transforms=wave_aug, output_type="object_dict"
        )
    ...
    def _preprocess(
        ...
        if (
            ...
            and self.preprocessing.spectrogram_conversion is not None
        ):
            ...
            if self.preprocessing.dbscale_conversion:
                ...
                spectrograms = self.preprocessing.dbscale_conversion(
                    spectrograms
                )  # different results??

            if self.preprocessing.resizer:
                spectrograms = self.preprocessing.resizer.resize_spectrogram_batch(
                    spectrograms
                )

            if self.preprocessing.normalize_spectrogram:
                spectrograms = (
                    spectrograms - self.preprocessing.mean
    ...
    def _zero_mean_unit_var_norm(self, input_values, attention_mask, padding_value=0.0):
        # w2v2
    ...
    def _min_max_scaling(self, input_values, attention_mask=None, padding_value=0.0):
    ...
    def _zero_center_and_peak_normalization(
        self,
        ...
        input_values = torch.where(
            peak_norm > 0.0, input_values / peak_norm, input_values
        )
    ...
    def _waveform_scaling(self, audio_augmented, attention_mask):
        ...
        if self.preprocessing.normalize_waveform == "instance_normalization":
            ...
            audio_augmented = self._zero_mean_unit_var_norm(
                input_values=audio_augmented, attention_mask=attention_mask
            )
